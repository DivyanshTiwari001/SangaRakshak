...
# img1_path = './image1.jpg'  # Path to the first image
# img2_path = './image4.jpg'  # Path to the second image
...
# Load and preprocess images (UPDATED)
def preprocess_image(img_path):
    # FIX: Resize to 28x28 to match model input shape
    img = image.load_img(img_path, target_size=(28, 28), color_mode="grayscale")
    img_array = image.img_to_array(img)
    # FIX: Add channel dimension and batch dimension in correct order
    img_array = np.expand_dims(img_array, axis=0)  # Add batch dimension
    img_array = img_array.astype("float32") / 255.0
    return img_array

# Define the base network (UPDATED input_shape)
def create_base_network(input_shape):
    model = models.Sequential()
    model.add(layers.Conv2D(32, (3, 3), activation='relu', input_shape=input_shape))
    model.add(layers.MaxPooling2D())
...
if os.path.exists(model_path):
    ...
    from tensorflow.keras.datasets import mnist
    (input_train, label_train), (input_test, label_test) = mnist.load_data()
    input_train = np.expand_dims(input_train, axis=-1)  # Add channel dimension
    input_train = input_train.astype("float32") / 255.0
    input_test = np.expand_dims(input_test, axis=-1)
    input_test = input_test.astype("float32") / 255.0
    ...
    input_shape = input_train.shape[1:]  # This will be (28, 28, 1)
...
# Function to compute similarity between two images (UPDATED)
def compute_similarity(img1_path, img2_path):
    img1 = preprocess_image(img1_path)  # Now returns (1, 28, 28, 1)
    img2 = preprocess_image(img2_path)  # Now returns (1, 28, 28, 1)
    
    # FIX: Remove the extra batch dimension added by preprocess_image
    # since the model expects pairs of (batch, 28, 28, 1)
    distance = siamese_model.predict([img1, img2])[0][0]
